...
try{
	...
	String sql = "SELECT * from interior where idx = " + idx;
...
<div id="wrapper">
		...
		<article class="post">
			...
			<footer>
				<ul class="actions">
					<li><a href="interior_delete_ok.jsp?idx=<%=rs.getString("idx") %>" class="button big">게시글 삭제</a></li>
					<!--게시글 삭제 버튼을 누르면 비밀번호를 적어서 확인하는 delete_ok.jsp로 이동한다.-->
					<li><a href="interior_modify_write.jsp?idx=<%=rs.getString("idx") %>" class="button big">게시글 수정</a></li>
					<!--게시글 수정 버튼을 누르면 게시글을 수정할 수 있는 modify_write.jsp로 이동한다.-->
					<li><a href="interior.jsp" class="button big">게시글 목록으로</a></li>
...
<%	
	...
	sql = "UPDATE interior SET COUNT=" + count + " where IDX=" +idx; 
...
%>
		<%if(session.getAttribute("id")!=null){ %>
		<form action="interior_comment.jsp?idx=<%=idx%>" method="post" onsubmit="return formCheck();">
