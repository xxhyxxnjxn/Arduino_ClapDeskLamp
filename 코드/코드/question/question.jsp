	...
	<!-- 메인 포스트 시작 -->
		<article class="post">
			<header>
				<!--글 제목-->
				<div class="title">
					<h2><a href="question_content.jsp?idx=<%=rs.getString("idx") %>"><%=rs.getString("title") %></a></h2>
				</div>
			...
			<footer>
				<ul class="actions">
					<li><a href="question_content.jsp?idx=<%=rs.getString("idx") %>" class="button big">Continue Reading</a></li>
				</ul>
			</footer>
		</article>
<%
...
%>
	...
	<ul class="actions">
		<li><a href="question_write.jsp" class="button big">글쓰기</a></li>
	</ul>
</div>
	<!-- 사이드 바를 생성한다. -->
	<section id="sidebar">
		...
		<!-- 이 메뉴 소개를 나타내는 글을 적어준다. -->
			...
			try{
				...
				//accessories의 테이블에 idx값을 오름차순으로 하여 모든 레코드를 선택한다.
				String sql2 = "SELECT * from question order by idx desc";
						...
						<!-- Mini Post -->
							<article class="mini-post">
								<header>
									...
									<h3><a href="question_content.jsp?idx=<%=rs2.getString("idx") %>"><%=rs2.getString("title") %></a></h3>
									<!--작성일과 작성자를 나타내준다.-->
